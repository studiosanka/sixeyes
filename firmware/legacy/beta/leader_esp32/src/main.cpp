...
bool readJointDegrees(float out_degrees[NUM_JOINTS], uint8_t out_valid[NUM_JOINTS]) {
  ...
  for (size_t i = 0; i < NUM_JOINTS; ++i) {
    int32_t accum = 0;
    for (int s = 0; s < 8; ++s) {
      accum += analogRead(JOINT_ADC_PINS[i]);
    }
    const int raw = static_cast<int>(accum / 8);
...
void publishJointState() {
  if (!zero_captured) {
    static uint32_t last_remind_ms = 0;
    if ((millis() - last_remind_ms) >= 2000) {
      Serial.println("[WAIT] Send CAPTURE_ZERO or HOME_ZERO to begin streaming");
      last_remind_ms = millis();
    }
    return;
  }
